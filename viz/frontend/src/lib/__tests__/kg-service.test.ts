...
import { fetchPipeline, fetchStage, queryGraphRAG, fetchStageItems } from '../services/kg-service';
...
describe('fetchStageItems', () => {
  it('calls /api/kg/stage/{n}/items with pagination params', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      '/api/kg/stage/1/items?page=2&page_size=10&q=foo',
      expect.anything(),
    );
  });
  ...
  it('omits q when empty', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      '/api/kg/stage/3/items?page=1&page_size=20',
      expect.anything(),
    );
  });
});

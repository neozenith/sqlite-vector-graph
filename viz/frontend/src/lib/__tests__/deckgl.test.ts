...
import type { ScatterplotDatum } from '../transforms/deckgl'
...
describe('toScatterplotData', () => {
  ...
  it('highlights search results in orange with high opacity', () => {
    const results: SearchNeighbor[] = [{ id: 2, distance: 0.1, label: 'b', metadata: {} }]
  ...
  it('selected takes priority over search highlight', () => {
    const results: SearchNeighbor[] = [{ id: 1, distance: 0.05, label: 'a', metadata: {} }]

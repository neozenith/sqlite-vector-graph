...
export function fetchSubgraph(
  ...
  return fetchJSON<SubgraphResponse>(
    `/api/graph/${encodeURIComponent(edgeTable)}/subgraph?limit=${limit}`,
  );
}
...
export function fetchBFS(
  ...
  return fetchJSON<BfsResponse>(
    `/api/graph/${encodeURIComponent(edgeTable)}/bfs?${params}`,
  );
}

...
describe('fetchSubgraph', () => {
  it('calls with limit parameter', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/graph/edges/subgraph?limit=200', expect.anything())
  })

  it('defaults to limit=500', async () => {
    mockOk({ nodes: [], edges: [] })
    await fetchSubgraph('edges')
    expect(mockFetch).toHaveBeenCalledWith('/api/graph/edges/subgraph?limit=500', expect.anything())
  })
})
...
describe('fetchCommunities', () => {
  it('calls with resolution', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/graph/edges/communities?resolution=1.5', expect.anything())
  })
})

...
describe('fetchEmbeddings', () => {
  it('calls /api/vss/{name}/embeddings with dimensions', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/test/embeddings?dimensions=3', expect.anything())
  })

  it('defaults to 2 dimensions', async () => {
    mockOk({ index: 'test', count: 0, points: [] })
    await fetchEmbeddings('test')
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/test/embeddings?dimensions=2', expect.anything())
  })

  it('encodes special characters in index name', async () => {
    mockOk({ index: 'my index', count: 0, points: [] })
    await fetchEmbeddings('my index')
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/my%20index/embeddings?dimensions=2', expect.anything())
  })
})
...
describe('searchVSS', () => {
  it('calls /api/vss/{name}/search with params', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/test/search?query_id=1&k=10&ef_search=32', expect.anything())
  })

  it('uses default k and ef_search', async () => {
    mockOk({ index: 'test', query_id: 1, k: 20, count: 0, neighbors: [] })
    await searchVSS('test', 1)
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/test/search?query_id=1&k=20&ef_search=64', expect.anything())
  })
})
...
describe('searchVSSByText', () => {
  it('calls /api/vss/{name}/search_text with query', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/vss/test/search_text?q=hello&k=10&ef_search=32', expect.anything())
  })

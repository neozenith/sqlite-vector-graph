...
export const vssKeys = {
  ...
  embeddings: (name: string, dims: number) =>
    ['vss', name, 'embeddings', dims] as const,
  search: (name: string, queryId: number, k: number) =>
    ['vss', name, 'search', queryId, k] as const,
  textSearch: (name: string, query: string, k: number) =>
    ['vss', name, 'text-search', query, k] as const,
};
...
export function useVSSTextSearch(
  indexName: string | null,
  query: string,
  k: number = 20,
) {
...
/** Combined hook for the VSS Explorer component — reads from Zustand store. */
export function useVSSExplorer() {
